...
// delete the initial contents of the table by dropping it and creating a new one with the same specs:
    ...
    if (err) {
      ...
      mysqlPool.query(' CREATE TABLE products \n ( manufacturer INT(255), bigbuy_id INT(255) NOT NULL, sku TEXT(255) NOT NULL, ean13 TEXT NOT NULL, weight DECIMAL, height DECIMAL, width DECIMAL, depth DECIMAL, dateUpd TEXT, category TEXT, categories TINYTEXT, dateUpdDescription TEXT, dateUpdImages TEXT, dateUpdStock TEXT, wholesalePrice DECIMAL, retailPrice DECIMAL, dateAdd TEXT, video TINYTEXT,  active TINYINT, images TINYTEXT, attributes TINYTEXT, tags TINYTEXT, taxRate INT(255), taxId INT(255), dateUpdProperties TEXT, dateUpdCategories TEXT, inShopsPrice DECIMAL, PRIMARY KEY (bigbuy_id)) ENGINE=INNODB;', (err, results) => {
        if (err) {
          console.error(err);
        } else {
